...
![Mount fuji](../../Imgs/Mount%20fuji.jpg)





不知不觉已在软件测试领域工作了近4年，现在将工作面试中经常遇到的软件测试面试题整理在成文档，也算是温故而知新吧😁
...
1）姓名、来自哪里、年龄、毕业院校、专业、从事测试工作X年

2）从介绍项目开始

- 介绍整个项目的主要业务和关键模块功能
- 以上就是项目的介绍，从项目开始阶段，严格按照测试流程进行测试
...
- 测试计划制定
- 编写测试用例
  - 测试用例话术：以上就是编写测试用例的常见方式方法
  - 当然用例还可以在一些工具上进行，我们是在华为的云龙上写测试用例
- 测试执行
  - 单元测试
  - 集成测试
    - 接口测试：接口测试话术
...
![img](../../Imgs/AgAABd_G7rcZbDq202xOPb12NfQwsjjl.png)
...
![img](../../Imgs/AgAABd_G7rcJjOqp3ptPnYb5MTtOHJbW.png)

Q：说一下你对测试用例的理解？在工作中怎么取编写测试用例？
...
对于测试常用的方法，一般有这么常用的几种，有等价类划分法，	就是一类信息，我们在测试的时候，只测试一种，没有必要所有的都进行测试。还有像边界值法，一般注册登录的时候，或者涉及到数学测试的时候，会用到。我项目中就用到了边界值测试法，比如需要上传学生成绩信息，做数据分析，学生成绩的测试用例，就牵扯到边界值法。还有一些场景法，设定不同的场景，不同场景就会有不同的操作。
...
![img](../../Imgs/AgAABd_G7rcHAGMm1vtHcZG7HGRvt_9q.png)
...
![img](../../Imgs/AgAABd_G7rfZmxyHWDdB25B700WtlLDD.png)

## 4.1 兼容性测试

![image-20230514150825652](../../Imgs/image-20230514150825652.png)
...
![image-20230514150859907](../../Imgs/image-20230514150859907.png)
...
![img](../../Imgs/AgAABd_G7reHgOh6gFRHyq10kdNS8oB4.png)
...
![img](../../Imgs/AgAABd_G7rc0SIDwwPpEi6VhxC0cS-JK.png)
...
![img](../../Imgs/AgAABd_G7reQiK-5lCJLPqhIcMQ3E0E-.png)
...
![img](../../Imgs/AgAABd_G7rfZy-Q9IZ9J67bs4IW7VmiH.png)
...
![img](../../Imgs/AgAABd_G7reGi5EPamRH0bV_rzFL1DK3.png)
...
![img](../../Imgs/AgAABd_G7rcesp6866dFyqc1FdEjGG0U.png)
...
在执行loadRunner脚本录制的时候，有一个比较重要的问题需要注意，就是用户会话问题，有些模块，在测试的时候，会生成一个会话id，比如登录功能，当用户登录成功之后，会为用户生成一个唯一的session ID，用户后续的操作，都是基于这个session ID，那我们如果使用loadRunner录制时所生成的那个sessionID，后续的操作就会报错。LoadRunner在执行回放的时候，可以设置值得关联，将session ID设置成动态的，当回访脚本，会使用最新的session ID，来避免这个问题。
...
![img](../../Imgs/AgAABd_G7rciF6UJ1RxPo7f-1Sa8clKN.png)
...
![img](../../Imgs/AgAABd_G7rcmapIxhRhCPrrsh-MD1_pG.png)
...
![img](../../Imgs/AgAABd_G7rflPZ4XDIFC6r0Oo47Q7u9_.png)

## 16.7 开发模型与测试模型

周期模型（典型的几种）：

![img](../../Imgs/AgAABd_G7re8QQnKHyFL5qorzeK70O0V.png)
...
![img](../../Imgs/AgAABd_G7rchY7BYqypN5oXXlZhnflBz.png)
...
![img](../../Imgs/AgAABd_G7rfwQfnyXsJB5p-ZYSZhCP2V.jpeg)
...
![img](../../Imgs/AgAABd_G7rdBnmmBRCpKMJmqIwgbk16c.png)
...
![img](../../Imgs/AgAABd_G7rfVPBQDPHRD3IiY8pRmJtCS.png)
...
![img](../../Imgs/AgAABd_G7reVs9g_4W5MKIJeX6w0PlUf.png)
...
![img](../../Imgs/AgAABd_G7re3SFoJb9lPFIj8_jr68Nv-.png)
...
​        ---mac电脑 xcode开发工具 去网上找到monkey测试的开源代码 在xcode上运行起来就可以对ios进行monkey测试 
...
- android SDK=software develop kit ，android软件开发工具包    
...
2. 在具体的操作过程中，在甲系统增加，修改A数据后，然后在乙系统查看对应的数据是否同步一致；反之亦然；
   ...
   - 甲系统传输过来的数据和乙系统接收到的数据是否一致；
   - 乙系统接收到数据后，会存入到自己的数据库，这个存储过程是否成功？（主要是考虑到甲系统传过来的数据格式是否和乙系统的格式一致）且数据存储成功与否乙系统会返回一条信息（例如：返回1，表示数据正确传输并存储到数据库了；返回0及错误信息表示数据传输或存储出了问题）
   - 然后在乙系统的界面查看，新增或修改的数据是否和接收到的数据一致；
...
7、请求url URL地  
8、请求参  
9、前置条件 执行当前请求依赖的条件，不满足就不能正确执  
10、结果验证 预期结  
11、请求报文 可以不  
12、返回报文 一定要写，这里应该是你请求返回的真实结  
13、测试结果 通过/失败  
14、测试人员  
...
- GIT是分布式的，SVN不是：这是GIT和其它非分布式的版本控制系统最核心的区别。
...
cd ..	切换到上一层目录 
cd /		切换到系统根目录
ls   显示当前目录想所包含的文件和文件夹 
ls -l 缩写成
ll 文件和目录的详情信息（不包含隐藏文件）
...
/ 查找
ifconfig 查看ip
grep addr nene.txt 在文件中查找 addr字符串
grep addr nene.txt --color 高亮显示
ps -ef |grep 2251 搜索进程号
ping 192.168.0.1 查看ip是否通畅
...
## 25.12 吞吐量、QPS、并发数、响应时间（RT）概念

[参考链接](https://www.cnblogs.com/data2value/p/6220859.html)
...
公共参数实际上就是接口中的一些公共参数，我上家公司接口中的公共参数有appVersion版本号，deviceName设备名称，source，authSign签名信息等这样的字段。所以在性能测试的过程中要把这些参数进行设置
...
![img](../../Imgs/AgAABd_G7rfOIZte0-5HZp4YOI3eKLQc.png)
...
![img](../../Imgs/AgAABd_G7reBBKmPc61O-LtN6TnehRxN.jpeg)
...
![img](../../Imgs/AgAABd_G7rfa2y5CVG5C_rk8KMubWsyW.png)

## 26.6 Bug单内容

![img](../../Imgs/AgAABd_G7rdesmeghm1MHbww0MROGa4L.jpeg)
...
![img](../../Imgs/AgAABd_G7re3SPxWjShCA74w6bQKUbwN.jpeg)
...
![img](../../Imgs/AgAABd_G7rcFggFlXFVIa5qkwmwlrkfS.png)
...
1我进去的时候项目已经上线了，一直存在，然后就是版本的微小更新，小修改的话，大概半个月一个版本，中修改的话，大概一个月一个版本。每次版本更新，针对新的功能点或者修改点大概写了60条案例左右（一个月一个版本的例子）。 
...
如果大家说自己是从零到有参与的项目，那么6个月时间是从需求分析开始。需求书编写完成前，产品经理他们是要做很多前期准备工作，可能要花费3个月左右的时间。
...
至此，所有面试题已收集完毕，对于文中的面试题如果发现错误或者有更好的回答，请给我发 E-Mail。

*2023 年 05 月 14日 编辑于东莞*
